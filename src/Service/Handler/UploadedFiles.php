...
class UploadedFiles
{
    ...
    public function handle($entity)
    {
        ...
        $regex = '#\<img.*?src="(?:http[s]*:\/\/[0-9\.a-z:]+)*('.preg_quote($publicUploadPath).
            '\/([^"]+\.[a-z]+))".*?\/>#i';

        // Match images in description
        if (!empty($entity->getDescription())
            && preg_match_all($regex, $entity->getDescription(), $matches)
        ) {
            foreach ($matches[2] as $fileName) {
                ...
                if ($file->isReadable()) {
                    ...
                    $replacedDescription = str_replace(
                        $publicUploadPath.'/'.$fileName,
                        $targetPublicPath.'/'.$fileName,
